...
UAC1_USB::UAC1_USB()
{
	...
	memset((&m_standart_config_desc+sizeof(m_standart_config_desc._header)), 0, sizeof(m_standart_config_desc));
	memset((&m_audio_contro_standart_ifc_desc+sizeof(m_audio_contro_standart_ifc_desc._header)), 0, sizeof(m_audio_contro_standart_ifc_desc));
...
inline void UAC1_USB::data_in(const uint8_t ep, uint8_t state, void* ptr)
{
	...
	if(ep != 2)
		...
		if(fifo_space_occupied(incnt,outcnt,f_size) > 8000)
			__asm__("bkpt");

		auto size = ept->trans_count;
/*
		if((incnt+size)>=f_size)
				{
					dotherest = (incnt+size)-f_size;
					offset = ((uint32_t)ept->cache)+(size-dotherest);

					_dma_set_destination_address(0,  (const void *) (cache+incnt));
					_dma_set_source_address(0, ept->cache);
					_dma_set_data_amount(0, (size-dotherest)/4);
		...
		if((incnt+size)>=f_size)
		{
			...
			memcpy(( void*)(cache+incnt), (const void*)ept->cache , size-from0 );
			memcpy(( void*)(cache), (const void*)(ept->cache+(size-from0)) ,from0);
			incnt = from0;
		}else
		{
			memcpy( ( void*)(cache+incnt), ( const void*)ept->cache, size );
			incnt = incnt + size;
		}

		//for(int i=0;i<size;i+=2)
				//fifo_in((((int32_t)ept->cache[i+1])<<24)|(((int32_t)ept->cache[i])<<16),(cache),incnt,f_size);
		//maxd++;
...
void UAC1_USB::init_start()
{
	...
	single_desc.eod = g_UsbDeviceConfigurationDescriptor + sizeof(g_UsbDeviceConfigurationDescriptor);
...
bool UAC1_USB::setup_data(const uint8_t ep, const enum usb_xfer_code rc, const uint32_t count)
{
...
int32_t  UAC1_USB::handle_control_rq(struct usbdf_driver *drv, enum usbdf_control ctrl, void *param)
{
...
int32_t UAC1_USB::handle_class_rq(uint8_t ep, struct usb_req *req, enum usb_ctrl_stage stage)
{
	ASSERT(m_this->m_started);
 ...
 */
int32_t UAC1_USB::device_enable(struct usbdf_driver *drv, struct usbd_descriptors *desc)
{
	m_this->no = true;
 ...
 * \return Operation status.
 */
int32_t UAC1_USB::usb_dev_set_req(uint8_t ep, struct usb_req *req, enum usb_ctrl_stage stage)
{
 ...
 * \return Operation status.
 */
int32_t UAC1_USB::usb_dev_get_req(uint8_t ep, struct usb_req *req, enum usb_ctrl_stage stage)
{
	uint16_t len = req->wLength;
...
int32_t UAC1_USB::usb_dev_disable(struct usbdf_driver *drv, struct usbd_descriptors *desc)
{
	m_this->no = true;
	...
	if (desc) {
		...
		// Check interface
		if ((ifc_desc.bInterfaceClass != USB_AUDIO_CLASS) && (ifc_desc.bInterfaceClass != USB_AUDIO_CLASS)) {
			return ERR_NOT_FOUND;
		}
	}
